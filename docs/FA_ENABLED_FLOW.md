...
- The flow step is stored in `sessionStorage` under `fa-enabled-flow-step` when advancing in the flow modal and **before** navigating to **Balances** (View / left nav). Returning to **Home** (`index.html`) via a normal navigation restores **Version 2 + FA enabled** to that step. **A full browser refresh** on Home clears the key and resets to **step 1** so the flow can be run again. **A new tab** or clearing site data starts at step **1** when there is no key.
